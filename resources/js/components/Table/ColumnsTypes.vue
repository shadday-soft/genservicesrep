<template>
  ...
  <div v-else>
    ...
    <a v-else-if="type == 'document'" :href="String(data)" target="_blank" rel="noopener noreferrer" class="text-blue-500 italic">Ver Documento</a>
    <span v-else-if="type == 'tag'">
      <Tag
        v-if="column?.tags && column.tags.find(tag => tag.value === data)"
        :severity="column.tags.find(tag => tag.value === data)?.severity"
        :value="column.tags.find(tag => tag.value === data)?.label"
      ></Tag>
