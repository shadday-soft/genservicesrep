    ...
    <template>
        <div>
            <form @submit.prevent="tecnicoService.submit(() => emit('close'))" class="grid grid-cols-1 md:grid-cols-3 gap-4">
                <!-- Foto -->
                <div class="col-span-full md:col-span-3 flex justify-between w-full">
                    <FileUpload
                        mode="basic"
                        name="foto"
                        accept="image/*"
                    ...
                    <p v-if="props.tecnico?.foto" class="mt-2 text-xs text-gray-600 flex flex-col items-center">
                        <img :src="'uploads/' + props.tecnico.foto" alt="Foto actual" class="size-20 rounded-full">
                        <p class="text-xs italic mt-1">Foto Actual</p>
