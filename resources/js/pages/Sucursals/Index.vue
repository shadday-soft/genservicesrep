...
import type { User, BreadcrumbItem, Sucursal, PaginatedResponse } from "@/types";
...
interface Props {
    sucursals: PaginatedResponse<Sucursal>;
    filters?: {
        search?: string;
        per_page?: number;
    };
...
<template>
    <AppLayout :breadcrumbs="breadcrumbs">
        ...
        <div>
            ...
            <Datatable :columns="columns" :data="sucursals.data" :noShowHeader="true">
                <template #addButton>
            ...
            <div class="mt-6 flex items-center justify-between border-t border-gray-200 dark:border-gray-700 pt-4">
                <div class="flex items-center gap-2 text-sm text-gray-600 dark:text-gray-400">
                    ...
                    <span class="font-semibold text-gray-900 dark:text-white">
                        {{ (sucursals.current_page - 1) * sucursals.per_page + 1 }}
                    </span>
                    <span>a</span>
                    <span class="font-semibold text-gray-900 dark:text-white">
                        {{ Math.min(sucursals.current_page * sucursals.per_page, sucursals.total) }}
                    </span>
                    <span>de</span>
                    <span class="font-semibold text-gray-900 dark:text-white">{{ sucursals.total }}</span>
                    <span>sucursales</span>
                </div>

                <Paginator 
                    :rows="sucursals.per_page"
                    :totalRecords="sucursals.total"
                    :first="(sucursals.current_page - 1) * sucursals.per_page"
